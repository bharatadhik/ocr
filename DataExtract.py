...
def coTox(img,L_id,name,X,Y,W,H,file_name,page_n,option,dformat):
    ...
    extracted_data={"folder_name":file_name.split("/")[0],"filename":file_name,"Page_n":page_n,"id":L_id,"field_name":name,"label_data":data,"Format":dformat}
    #except:
...
def Main(img_name,file_name,page_n,option):
    ...
    with open('out.csv','r') as csv_file:
        ...
        for line in csv_reader:
            if img_name==line[0]:
                try:
                    All_Data[line[1]]=coTox(f"./images/{img_name}",line[1],line[2],int(line[3]),int(line[5]),int(line[4]),int(line[6]),file_name,page_n,option,line[7])
                except:
                    pass
...
def MainImg(img_name,file_name,page_n,option):
    ...
    with open('out.csv','r') as csv_file:
        ...
        for line in csv_reader:
            try:
                All_Data[line[1]]=coTox(img_name,line[1],line[2],int(line[3]),int(line[5]),int(line[4]),int(line[6]),file_name,page_n,option,line[7])
            except:
                pass
    return All_Data

        ...
        for f in files:
            ...
            if extention in [".jpg", ".png", ".tiff", ".tif"]:
                ...
                pages = convert_from_path(
                    ...
                    poppler_path=r"D:\tmp\DEA03Jan2022\DEA\poppler-0.67.0_x86\bin",
                )
...
def download():
    ...
    ############################# added this
    print("✅✅✅")
    df = pd.DataFrame.from_dict(app.config["Data"][0], orient="index")
    #################3
    new_df = df.pivot(
        index=["folder_name", "filename", "Page_n", "id", "Format"],
        columns="field_name",
        values="label_data",
    ).reset_index()
    ...
    # output_list = [json.loads(item) for item in json_list]
